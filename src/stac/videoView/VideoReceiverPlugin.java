...
public abstract class VideoReceiverPlugin extends Thread {
	...
	public void setImage(BufferedImage image) {
		if (image!=null) {
			...
			if (imageAct.img!=null) {
				int RGBval=255;
				int w=imageAct.img.getWidth();
				int h=imageAct.img.getHeight();
//				{
//					// box
//					w/=10;
//					h/=10;;
...
//					for(int i =0;i<w*h;i++)
//						RGBdata[i]=RGBval;
//					imageAct.img.setRGB(0, 0, w, h, RGBdata, 0, w);
//				}
				{
					// X
					for (int i = 0; i<h;i++) {
						try {
							imageAct.img.setRGB((int)((double)w/h*i), i, RGBval);
						} catch (Exception e) {
						}
						try {
							imageAct.img.setRGB((int)(w-(double)w/h*i), i, RGBval);
						} catch (Exception e) {
						}
					}
				}
