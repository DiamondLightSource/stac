...
public class Stac_BCMplugin_tango extends Stac_BCMplugin_base implements Stac_BCMplugin, fr.esrf.TangoDs.TangoConst {
	...
	// TANGO device stuff
	fr.esrf.TangoApi.DeviceProxy device;
...
//	
    ...
    double tango_getAttr (String attr_name){
    	try {
			return tango_getAttrDA(attr_name).extractDouble();
		} catch (Exception e) {
		}
		return 0;
    }
    	
    String tango_getAttrStr (String attr_name){
    	try {
			return tango_getAttrDA(attr_name).extractString();
		} catch (Exception e) {
		}
		return "";
    }
    	
    DeviceAttribute tango_getAttrDA (String attr_name){
    	return tango_getAttrDA(attr_name,tango_AttrSuffix());
    }
    
    DeviceAttribute tango_getAttrDA (String attr_name,String AttrSuffix){
    	
    	DeviceAttribute ret_client_string = null;
...
//    		ret_client_string = device.read_attr(attr_name+tango_AttrSuffix());
			ret_client_string = device.read_attribute(attr_name+AttrSuffix);
		}
//    	catch(jive.DevHelperDevFailed e)
//		{
//    		StringBuffer errmsg = new StringBuffer();
...
//    		ret_client_string = device.write_attr(attr_name+tango_AttrSuffix(),""+newValue);
    		device.write_attribute(new DeviceAttribute(attr_name+tango_AttrSuffix(),newValue));
    		boolean inMove=true;
...
//		}
    ...
    String tango_CMD (String cmd_name, String cmd_param){
    	try {
			return tango_CMD_DA(cmd_name,cmd_param).extractString();
		} catch (Exception e) {
		}
		return "";
    }

   	DeviceData tango_CMD_DA (String cmd_name, String cmd_param){
    	
    	DeviceData ret_client_string = null;
		...
		{
//    		device.set_source( new Integer(config.Descriptor.getFirstStringValue("SOURCE")).intValue() );
    		DeviceData par = new DeviceData();
    		par.insert(cmd_param);
    		ret_client_string = device.command_inout(cmd_name,par);
		}
//    	catch(jive.DevHelperDevFailed e)
//		{
//    		StringBuffer errmsg = new StringBuffer();
...
//		}
    ...
    AttrInfo tango_getAttrProperties (String attr_name) throws Exception{
		...
		fr.esrf.TangoApi.AttributeInfo[] tango_attr_configs=null;
    	try {
			attrs = device.get_attribute_list();
			// Get for each attribute all its properties
			tango_attr_configs = device.get_attribute_config();
		} catch (Exception e) {
			Stac_Out.println("Error when retrieving the Attribute configurations for "+attr_name);
			return null;
		}
    	...
    	{
    		if(!(attr_name+tango_AttrSuffix()).equals(tango_attr_configs[i].name))
    			continue;
    ...
    public double convertMotorPosition_Stac2Plugin(String motorName,double stacPos) {
    	...
    	if (motorName.equalsIgnoreCase("Omega")||
    			...
    			try {
    				...
    				double offset=params.getFirstDoubleValue("minpos");
    				if (Math.abs(offset)> 1e5) 
    					offset=0;
    				pluginPos-=offset;
