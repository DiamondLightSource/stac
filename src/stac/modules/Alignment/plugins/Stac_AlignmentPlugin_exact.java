...
//				double [] hkl2;
	  		...
	  		while (close && !CheckSingleAlignment(outdir+pos)){
	  			...
	  			if (rot<360 && testAngle!=0.0) {
	  				...
	  				//to show a bug in j3d 1.3.1:
	  				//Stac_Out.println("DEBUG: rot:"+rot+" aa:"+rot1+" m:"+rot1m+" orig:"+hkl2a+" new:"+hkl2);
	  			} else {
	  				rot=0;
	  				testAngle+=5;
...
//	  			neworientation.setElementAt(sepChar.substring(0,1)+hkl[0]+" "+hkl[1]+" "+hkl[2]+sepChar.substring(1,2),1);
	  			...
	  			if (testAngle>=90)
	  				break;
	  			//test it
	  			String subdir=outdir+"/suborientation"+new Integer(orientct).toString();
	    		File oDir= new File(subdir);
	    		...
	    		if (!stat) {
	    			session.addErrorMsg(msgId,"Could not create the output dir: "+subdir+"\n Use '"+outdir+"' instead.");
	    			subdir=outdir;
	    		}
	    		subdir=new File(subdir).getAbsolutePath()+"/";
		  		pos=CalculateExactAlignment(session,msgId,indir,subdir,datum,trans,neworientation,bcm,utils);
				try {
					utils.copyFile(new File(subdir+pos), new File(outdir+pos));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					Stac_Out.println( "Problem with copying the result file: "+subdir+pos);
				}
