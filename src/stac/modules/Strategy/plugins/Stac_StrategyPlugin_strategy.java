...
public class Stac_StrategyPlugin_strategy extends Stac_Plugin implements Stac_StrategyPlugin {
    ...
    public String CalculateStrategy(Stac_Session session,int msgId,String indir,String outdir,Vector3d currDatum,Vector DesiredOrientations,String options,StacBCM bcm,Util utils) {
    	...
    	for (int i=0;i<DesiredOrientations.size();i+=2) {
    		...
    		if (!selectStrategy(msgId,outdir,"STAC_strategy1.res",StrID,comment,kappa,phi,1,bcm)) {
    			...
    			selectStrategy(msgId,outdir,"STAC_strategy2.res",StrID,comment,currDatum.y,currDatum.z,0,bcm);
    			...
    			if (newKappa!=currDatum.y || newPhi!=currDatum.z) {
    				...
    				try {
						...
						StrID++;
						selectStrategy(msgId,outdir,"STAC_strategy1.res",StrID,comment,kappa,phi,1,bcm);
						selectStrategy(msgId,outdir,"STAC_strategy2.res",StrID,comment,newKappa,newPhi,0,bcm);
					} catch (Exception e) {
					}
    			}
        	}
    ...
    public boolean selectStrategy(int msgId,String workDir,String myfile,int strID,String comment,double kappa,double phi,int selectOption,StacBCM bcm) {
    	...
    	try {
    			...
    			do {
    				...
    				if (selectOption==1 || completionTemp>completionRef ||
    	    	       (completionTemp==completionRef &&  rangeRef>rangeTemp)) {
    					shadowComment="";
    	    			Vector newranges=bcm.avoidShadow(new Point3d(omegaStartTemp,kappa,phi),omegaStartTemp,omegaEndTemp,true);
    	    			if (newranges.size()>0) {
    	    				if (newranges.size()>2) {
        	    				shadowComment="Strategy sweep ("+omegaStartTemp+" - "+omegaEndTemp+";"+kappa+";"+phi+") is split to subsweeps to avoid shadow.";
    	    				}
    	    				//omegaStart = omegaStartTemp;
    	    				//omegaEnd   = omegaEndTemp;
    	    				omegaranges=newranges;
    	    				completion = completionTemp;
    	    				completionRef = completionTemp;
    	    				res = resTemp;
    	    				rangeRef      = rangeTemp;
    	    			} else {
    	    				shadowComment="Possible strategy sweep ("+omegaStartTemp+" - "+omegaEndTemp+";"+kappa+";"+phi+") is removed because of shadow.";
    	    			}
    				}
    		...
    		if (validValue) {
    			...
    			if (shadowComment.length()!=0) {
	 		    	session.addErrorMsg(msgId,shadowComment);
    			}
...
//	             double rank  = utils.AlignmentRanking(trans,omega,kappa,phi,bcm,CA);
    			...
    			for (int ct=0; ct< omegaranges.size();ct+=2) {
    				outFile.writeInt(strID);
    				outFile.writeDouble(((Double)omegaranges.elementAt(ct)).doubleValue());
    				outFile.writeDouble(((Double)omegaranges.elementAt(ct+1)).doubleValue());             
    				...
    				outFile.writeUTF(comment+((omegaranges.size()>2)?(" - shadow run #"+(ct/2+1)):("")));
    			}
