...
def d_min(detDiameter, detDistance, wavelength):
    return 1/(2*math.sin(math.atan((detDiameter/2.)/detDistance)/2.)/wavelength)
    
def read_xds_hkl(hkl_str, ftype = "INTEGRATE"):
    #get numofelements in a row in integrate 
    numofelements=19
    try:
        key="NUMBER_OF_ITEMS_IN_EACH_DATA_RECORD"
        ind=hkl_str.index(key)
        ind=ind+36 #key.len+1
        numofelements=int(hkl_str[ind:ind+2])
    except:
        numofelements=19
    ...
    _len = len(hkl_num)
    if ftype == "INTEGRATE":hkl_num.shape = (_len/numofelements,numofelements)
...
def PARS_xds2dnz(xdsPar):
    ...
    dnz['cell'] = 6*"%9.3f" % tuple(xdsPar["cell"])
...
if __name__=='__main__':
    ...
    if _inputReflectionName and os.path.isfile(_inputReflectionName):
        ...
        #print hkl_xds[:,:]
        hkl_xds[:,3]= hkl_xds[:,3]*2.0
        hkl_xds[:,4]= hkl_xds[:,4]*2.0
        #print hkl_xds[1,4], hkl_xds[1,5]
